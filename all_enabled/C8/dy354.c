...
void F505_3470 (EIF_REFERENCE Current, EIF_NATURAL_16 arg1)
{
	GTCX
	RTEX;
	...
	RTEAA("put_left", 504, Current, 1, 1, 6221);
	RTSA(Dtype(Current));
	RTSC;
	RTGC;
	RTIV(Current, RTAL);
...
body:;
	...
	if ((nstcall = 0, F290_2537(Current))) {
		RTHOOK(4);
		(nstcall = 0, F605_4111(Current, arg1));
	} else {
		RTHOOK(5);
		if ((nstcall = 0, F490_3332(Current))) {
			...
			RTHOOK(7);
			(nstcall = 0, F605_4116(Current, arg1));
			RTHOOK(8);
			(nstcall = 0, F605_4103(Current, ((EIF_INTEGER_32) 2L)));
		} else {
			RTHOOK(9);
			...
			RTHOOK(11);
			(nstcall = 0, F605_4116(Current, loc1));
			RTHOOK(12);
...
void F505_3472 (EIF_REFERENCE Current, EIF_REFERENCE arg1)
{
	GTCX
	RTEX;
	EIF_REFERENCE tr1 = NULL;
	...
	RTEAA("merge_left", 504, Current, 0, 1, 6218);
	RTSA(Dtype(Current));
	RTSC;
	RTGC;
	RTIV(Current, RTAL);
...
body:;
	...
	for (;;) {
		...
		if (tb1) break;
		RTHOOK(7);
		tu2_1 = (nstcall = 1, F605_4078(RTCW(arg1)));
		(nstcall = 0, F605_4115(Current, tu2_1));
		RTHOOK(8);
		(nstcall = 1, F605_4127(RTCW(arg1)));
	}
	if (RTAL & CK_ENSURE) {
		RTHOOK(9);
		RTCT("new_count", EX_POST);
...
void F505_3473 (EIF_REFERENCE Current, EIF_REFERENCE arg1)
{
	GTCX
	RTEX;
	EIF_REFERENCE tr1 = NULL;
	...
	RTEAA("merge_right", 504, Current, 0, 1, 6219);
	RTSA(Dtype(Current));
	RTSC;
	RTGC;
	RTIV(Current, RTAL);
...
body:;
	...
	for (;;) {
		...
		RTHOOK(7);
		tu2_1 = (nstcall = 1, F605_4078(RTCW(arg1)));
		(nstcall = 0, F605_4116(Current, tu2_1));
		RTHOOK(8);
		(nstcall = 1, F605_4107(RTCW(arg1)));
		RTHOOK(9);
		(nstcall = 1, F605_4130(RTCW(arg1)));
	}
	if (RTAL & CK_ENSURE) {
		RTHOOK(10);
		RTCT("new_count", EX_POST);
...
void F505_3477 (EIF_REFERENCE Current)
{
	GTCX
	RTEX;
	RTSN;
	...
	RTEAA("wipe_out", 504, Current, 0, 0, 6220);
	RTSA(Dtype(Current));
	RTSC;
	RTGC;
	RTIV(Current, RTAL);
	RTHOOK(1);
	(nstcall = 0, F475_3327(Current));
	RTHOOK(2);
	(nstcall = 0, F605_4107(Current));
	if (RTAL & CK_ENSURE) {
		RTHOOK(3);
